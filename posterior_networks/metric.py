...
def confidence(glod_label, alpha):
    corrects = (glod_label.squeeze() == alpha.max(-1)[1]).cpu().detach().numpy()
    # if uncertainty_type == 'epistemic':
    epis_scores = alpha.max(-1)[0]
    # elif uncertainty_type == 'aleatoric':
    p = torch.nn.functional.normalize(alpha, p=1, dim=-1)
    alea_scores = p.max(-1)[0]
    ...
    return corrects, p, epis_scores, alea_scores
...
def auc(corrects, scores):
        
    scores=reduce(lambda x,y:x+y,scores)
    scores = list(chain.from_iterable(scores))
    corrects=reduce(lambda x,y:x+y,corrects)
    corrects = list(chain.from_iterable(corrects))
