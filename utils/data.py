...
class Data:
    ...
    def write_decoded_results(self, predict_results, uncertainty, prob, name):
        ...
        for idx in range(sent_num):
            ...
            for idy in range(sent_length):
                ...
                fout.write(content_list[idx][0][idy]+ " " + str(uncertainty[idx][idy]) + " " + str(prob[idx][idy]) +' '+ predict_results[idx][idy] + '\n')
            fout.write('\n')
        fout.close()
        print("Predict %s result has been written intoo file. %s"%(name, self.decode_dir))
    ...
    def read_config(self,args):
        ...
        self.k_lipschitz = args.k_lipschitz
        self.density = str2bool(args.density)
        self.budget_function = args.budget_function
        ...
        self.HP_loss = args.HP_loss
        self.HP_regr = float(args.HP_regr)
        # word representation
        ...
        # architectures1 parameter
        self.HP_architectures1_dropout = float(args.architectures1_dropout)
        self.HP_hidden_dim = int(args.hidden_dim)
        self.HP_architectures1_layer = int(args.architectures1_layer)
        self.HP_bilstm = str2bool(args.bilstm)
        self.HP_threshold = float(args.threshold)

        # architectures2 parameter
        self.HP_label_embed_dim = int(args.label_embed_dim)
        self.HP_label_embedding_scale = float(args.label_embedding_scale)
        self.HP_architectures2_layer = int(args.architectures2_layer)
        self.HP_dhead = int(args.d_head)
        self.HP_nhead = int(args.n_head)
        self.HP_architectures2_dropout = float(args.architectures2_dropout)
        self.HP_attention_dropout = float(args.attention_dropout)
        self.d_architectures = self.HP_nhead * self.HP_dhead
        self.HP_dim_feedforward = int(2 * self.d_architectures)
